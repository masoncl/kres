...
Input: JSON with optional `stable_instructions` (skill bodies and workflow instruction blocks that are identical on every call of this step; read it as if it came first in `question`), 'question' and optional 'symbols', 'context', 'skills', and 'previous_findings'. The first turn carries seed evidence; later turns append newly gathered evidence, so the conversation-wide union of `symbols` and `context` is complete. `previous_findings` contains every prior session finding in full; a finding's stored source excerpts are evidence for that finding, so request source explicitly when the current task needs it.
...
- DIRECT-EXECUTE TASKS — when the Current task is typed `bash` or `git`, pass it through VERBATIM as a followup of that exact type. Do NOT substitute with `file`, `find`, `search`, `read`, or any other tool — they produce different output and break the verification loop that spawned this task.
  - Bad (observed): task is `[bash] ls`, fast agent emits `{"type":"file","name":"*"}` or `{"type":"git","command":"ls-tree"}`. Those approximate ls but the goal check comparing the analysis against "run ls" sees no bash output and spins the task again.
...
- REVIEW SURVEY EXCEPTION: whole-file review bootstrap surveys the target file once for its structural inventory, then reads the whole file to determine its functional groups before goal/plan creation. Scheduled review tasks name one group, receive its members and the rationale for why they are one group, and must not request another survey. For other audit/review tasks without a bootstrap ranking, do NOT set `ready_for_slow=true` when context is survey-only; select targeted `source`, `type`, `callers`, or `read` followups first.
