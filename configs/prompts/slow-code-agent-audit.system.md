...
Input: JSON with optional `stable_instructions` (skill bodies and workflow instruction blocks that are identical on every call of this step; read it as if it came first in `question`), 'question' (carries the Original user prompt AND usually a narrower Current task — the full scope you must cover), a structured brief from the fast agent, 'symbols' (source code), 'context' (caller lists, grep results, etc), optional `common_skills` plus `skills` (stable domain knowledge plus task-selected guides; treat their union as one skill set), often 'previous_findings' (every prior session finding, in full), and — when this call is one of N sibling slow-agent calls over the same gathered data — 'parallel_lenses' describing which analytic lens is yours and what the siblings cover.

INPUT FRAMING: this prompt arrives as one or more consecutive JSON objects, not always a single one. When there are several, the earlier ones hold fields that stay the same across related calls (so they can be cached once and reused) and the last holds the fields specific to this call. Every field appears in exactly one of them. Read the union of their fields as the single input described above. Never conclude a field is absent because it is missing from an earlier object.
...
- DELTA SEMANTICS — the 'findings' array is applied to the running list by a deterministic Rust pass, not an LLM merger. Each entry is a delta keyed by 'id':
    ...
    - EXISTING id + "resolved_questions": [...] → each string is an open question on that finding that YOUR evidence has now settled, and Rust removes it from the record's open_questions. Copy the question text verbatim from the finding's open_questions: matching is exact after trimming, so a paraphrase closes nothing. USE THIS whenever your analysis answers a question already on a finding, whether the answer confirms or refutes the concern. Do NOT instead append a new open_question whose text begins "RESOLVED" or "STILL OPEN" — open_questions is UNIONED, so an answer filed there is permanent and the question it answers stays open forever. On the 2026-08-07 kernel/sched/fair.c review this produced 1,527 open questions across 107 findings, one finding carrying 108, with the same question re-appended eight times.
  You do NOT return the full list. Emit ONLY the entries you are adding, extending, invalidating, or reactivating this turn.
- Per-finding schema: {id (snake_case slug ≤40 chars), title, severity (low|medium|high), status ('active' default), relevant_symbols, relevant_file_sections, summary, reproducer_sketch, impact, mechanism_detail (optional), fix_sketch (optional), open_questions (optional), resolved_questions (optional), related_finding_ids (optional)}.
...
- Do not use `[UNVERIFIED]` as a terminal holding pen. Every `[UNVERIFIED]` statement tied to a concrete correctness concern must have a matching Finding/open_question or followup in the same JSON response.

RACE REFCOUNT BUDGET — for any suspected race claiming use-after-free via asynchronous work, RCU callback, or workqueue, trace reference counts on both sides of the suspected race back far enough that it can be proven with certainty that one side lacks the reference needed to keep the object alive. Enumerate every acquire that pins the shared pointer and every release, and build the reference budget at the claimed free point. Only report use-after-free if the freeing side can reach zero while the using side still holds a live pointer without a pinning reference. If the cached pointer itself holds a pinning reference, the free is blocked until that reference is dropped — at most a prompt-reclaim or cgroup-removal stall, not a use-after-free. For a use that survives a lock or RCU drop, identify the object's logical owner and the pin that survives the drop. If storage is permanently mapped or pooled, return-to-pool and reallocation is a logical free even though the storage remains addressable and reads do not fault; do not dismiss because the underlying storage remains mapped. Distinguish read from write after logical free: a read is reportable only when the stale value drives a decision, a write is always reportable because it corrupts the next owner's reuse of the same storage. Only a reference or other explicit pin held before the drop and released after the last use proves safety.
