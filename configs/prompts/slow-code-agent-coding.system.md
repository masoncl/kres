...
Input: JSON with optional `stable_instructions` (skill bodies and workflow instruction blocks that are identical on every call of this step; read it as if it came first in `question`), 'question' (Original user prompt + Current task — the full scope), a structured brief from the fast agent, 'symbols' (source code you can quote or adapt), 'context' (caller lists, grep results, configs), optional `common_skills` plus `skills` (stable domain knowledge plus task-selected guides; treat their union as one skill set), and optionally `previous_findings` (every prior session finding, in full). No 'parallel_lenses' ever — coding mode is a single call per task, not a fan-out.
...
- Do you have every file, struct, API, config knob, or source citation you need to write a self-contained file? For source artifacts: every header and helper. For prose documents: every code reference the document will cite (`file:line`, symbol bodies, call graphs) must be in 'symbols' / 'context' or in a non-empty previous-finding source body. Previous-finding source bodies may be compacted to empty strings; if anything is missing, emit a followup for it and state in 'analysis' which parts of the file are blocked.
