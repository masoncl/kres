...
- DELTA SEMANTICS — the findings array is applied as a delta keyed by 'id' by a deterministic Rust pass, not an LLM merger. NEW id appends; EXISTING id (matching a 'previous_findings' entry) updates the existing record in place (union relevant_symbols / relevant_file_sections / related_finding_ids / open_questions, non-empty prose fields overwrite, severity only rises); EXISTING id with "status": "invalidated" flips the existing record to invalidated — use this when new context you just saw makes a prior finding wrong (guard you missed, bound already enforced, ordering actually honoured), and it MUST carry "invalidation": {"premise": "the one claim that, being true, means this is not a bug", "evidence": ["file.c:123"]} or Rust rejects the flip. When you cannot state such a premise, send "status": "unconfirmed" instead, which moves the record from active or invalidated with no flag and costs nothing to leave. Emit ONLY entries you are adding, extending, or invalidating this turn, never the full list.
