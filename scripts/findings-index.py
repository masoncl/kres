...
Two modes, picked by mutually exclusive flags:
  ...
  findings-index.py --generate
      ...
      write:
        * INDEX.md                — markdown table, all findings.
        * INDEX-<subsystem>.md    — one per distinct subsystem value.
        * index.html              — same table with client-side
                                    filters for browser / GitHub Pages
                                    viewing.
      The per-subsystem indexes are generated by default — they let
      operators pivot a large tree by area without scanning the whole
      list.
...
# --- per-subsystem index generation -----------------------------------


def sanitize_subsystem(name):
    """Turn an arbitrary subsystem string into a filename-safe slug.

    Non-alphanumeric / non-(`-`,`_`,`.`) characters collapse to `-`.
    Leading/trailing dashes are stripped. An all-symbols subsystem
    becomes `unknown`.
    """
    out = []
    last_dash = False
...
def build_subsystem_indexes(rows, root):
    """Write one INDEX-<subsystem>.md per distinct subsystem value.

    Rows with no subsystem are excluded from the per-subsystem files
    (they still appear in the umbrella INDEX.md). Returns the list of
    paths written. Cleans up stale `INDEX-*.md` files that no longer
    match a current subsystem so renaming a subsystem in metadata.yaml
    doesn't leave a dangling index.
    """
    grouped = {}
    for r in rows:
        sub = r["subsystem"]
        if not sub:
            continue
        grouped.setdefault(sub, []).append(r)

    written = []
    for sub, sub_rows in sorted(grouped.items()):
        slug = sanitize_subsystem(sub)
        path = os.path.join(root, "INDEX-{}.md".format(slug))
        with open(path, "w", encoding="utf-8") as f:
            f.write(build_markdown(sub_rows))
        written.append(path)

    # Garbage-collect stale per-subsystem indexes.
...
def main():
    ...
    if args.generate:
        ...
        sub_paths = build_subsystem_indexes(rows, root)
        print(
            "wrote {} and {} ({} row(s); {} per-subsystem index(es))".format(
                md_path, html_path, len(rows), len(sub_paths)
            ),
            file=sys.stderr,
        )
        return 0
